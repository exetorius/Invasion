...
void UEngineeringScreenWidget::InitialiseEngineeringScreen()
{
	...
	{
		GetWorld()->GetTimerManager().SetTimerForNextTick(this, &UEngineeringScreenWidget::InitialiseEngineeringScreen);
		return;
	}
...
void UEngineeringScreenWidget::OnAssignClicked(FGuid TaskID)
{
	...
	{				
		...
		{
			if (Worker->Role == EWorkerRole::EWRO_Engineer && Worker->CurrentStatus == EWorkerStatus::EWS_Idle)
			{
